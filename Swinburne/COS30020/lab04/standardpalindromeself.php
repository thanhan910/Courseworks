...
<body>
    ...
    if (isset ($_POST["myString"])) { // check if form data exists
        ...
        $replaced_str = str_replace([' ', '.', ',', '!', '?', ';', ':', '-', '_', '"', "'", '(', ')'], '', $lower_str); // remove all special characters
        ...
        if (strcmp($replaced_str, $reversed_str) == 0) { // check if the string is a palindrome
            ...
            echo "<p>The text you entered: '", htmlspecialchars($str, ENT_QUOTES), "' is a standard palindrome!</p>";

        } else { // string is not a palindrome
            echo "<p>The text you entered: '", htmlspecialchars($str, ENT_QUOTES), "' is not a standard palindrome.</p>";
        }
    } else { // no input
